export const englishKeyboardDisambiguations = {
    'b': [
        {
            turkishText: 'b',
            arabicText: 'ب',
            code: 'b'
        },
        {
            turkishText: 'p', // at the end of the word, automatically p
            arabicText: 'ب',
            code: 'b7'
        }
    ],
    'B': [
        {
            turkishText: 'B',
            arabicText: 'ب',
            code: 'B'
        },
        {
            turkishText: 'P',
            arabicText: 'ب',
            code: 'B7'
        }
    ],
    'c': [
        {
            turkishText: 'c',
            arabicText: 'ﺝ',
            code: 'c'
        },
        {
            turkishText: 'ç',
            arabicText: 'چ',
            code: 'c2'
        },
        {
            turkishText: 'ç',
            arabicText: 'ج',
            code: 'c7'
        }
    ],
    'C': [
        {
            turkishText: 'C',
            arabicText: 'ﺝ',
            code: 'C'
        },
        {
            turkishText: 'Ç',
            arabicText: 'چ',
            code: 'C2'
        },
        {
            turkishText: 'Ç',
            arabicText: 'ج',
            code: 'C7'
        }
    ],
    'd': [
        {
            turkishText: 'd', // default
            arabicText: 'د',
            code: 'd'
        },
        {
            turkishText: 'ḍ',
            arabicText: 'ض',
            code: 'd3'
        },
        {
            turkishText: 't',
            arabicText: 'د',
            code: 'd7'
        }
    ],
    'D': [
        {
            turkishText: 'D',
            arabicText: 'د',
            code: 'D'
        },
        {
            turkishText: 'Ḍ',
            arabicText: 'ض',
            code: 'D3'
        },
        {
            turkishText: 'T',
            arabicText: 'د',
            code: 'D7'
        }
    ],
    'f': [
        {
            turkishText: 'f',
            arabicText: 'ف',
            code: 'f'
        }
    ],
    'F': [
        {
            turkishText: 'F',
            arabicText: 'ف',
            code: 'F'
        }
    ],
    'g': [
        {
            turkishText: 'ġ', // default
            arabicText: 'غ',
            code: 'g4'
        },
        {
            turkishText: 'ğ',
            arabicText: 'غ',
            code: 'g5'
        },
        {
            turkishText: 'g',
            arabicText: 'ك',
            code: 'g6'
        },
        {
            turkishText: 'k',
            arabicText: 'ك',
            code: 'g7'
        }
    ],
    'G': [
        {
            turkishText: 'Ġ',
            arabicText: 'غ',
            code: 'G4'
        },
        {
            turkishText: 'Ğ',
            arabicText: 'غ',
            code: 'G5'
        },
        {
            turkishText: 'G',
            arabicText: 'ك',
            code: 'G6'
        },
        {
            turkishText: 'K',
            arabicText: 'ك',
            code: 'G7'
        }
    ],
    'h': [
        {
            turkishText: 'h',
            arabicText: 'ه',
            code: 'h'
        },
        {
            turkishText: 'ḥ', // default
            arabicText: 'ح',
            code: 'h3'
        },
        {
            turkishText: 'ẖ',
            arabicText: 'خ',
            code: 'h5'
        }
    ],
    'H': [
        {
            turkishText: 'H',
            arabicText: 'ه',
            code: 'H'
        },
        {
            turkishText: 'Ḥ',
            arabicText: 'ح',
            code: 'H3'
        },
        {
            turkishText: 'Ḫ',
            arabicText: 'خ',
            code: 'H5'
        }
    ],
    'j': [
        {
            turkishText: 'j',
            arabicText: 'ژ',
            code: 'j'
        }
    ],
    'J': [
        {
            turkishText: 'J',
            arabicText: 'ژ',
            code: 'J'
        }
    ],
    'k': [
        {
            turkishText: 'k', // default
            arabicText: 'ك',
            code: 'k'
        },
        {
            turkishText: 'ḳ',
            arabicText: 'ق',
            code: 'k3'
        }
    ],
    'K': [
        {
            turkishText: 'K',
            arabicText: 'ك',
            code: 'K'
        },
        {
            turkishText: 'Ḳ',
            arabicText: 'ق',
            code: 'K3'
        }
    ],
    'l': [
        {
            turkishText: 'l',
            arabicText: 'ل',
            code: 'l'
        }
    ],
    'L': [
        {
            turkishText: 'L',
            arabicText: 'ل',
            code: 'L'
        }
    ],
    'm': [
        {
            turkishText: 'm',
            arabicText: 'م',
            code: 'm'
        }
    ],
    'M': [
        {
            turkishText: 'M',
            arabicText: 'م',
            code: 'M'
        }
    ],
    'n': [
        {
            turkishText: 'n', // default
            arabicText: 'ن',
            code: 'n'
        },
        {
            turkishText: 'ñ',
            arabicText: 'ك',
            code: 'n6'
        }
    ],
    'N': [
        {
            turkishText: 'N',
            arabicText: 'ن',
            code: 'N'
        },
        {
            turkishText: 'Ñ',
            arabicText: 'ك',
            code: 'N6'
        }
    ],
    'p': [
        {
            turkishText: 'p', // default, add b7 option
            arabicText: 'ﭖ',
            code: 'p'
        }
    ],
    'P': [
        {
            turkishText: 'P',
            arabicText: 'ﭖ',
            code: 'P'
        }
    ],
    'r': [
        {
            turkishText: 'r',
            arabicText: 'ر',
            code: 'r'
        }
    ],
    'R': [
        {
            turkishText: 'R',
            arabicText: 'ر',
            code: 'R'
        }
    ],
    's': [
        {
            turkishText: 's', // default
            arabicText: 'س',
            code: 's'
        },
        {
            turkishText: 'ş', // sh --> s2???
            arabicText: 'ش',
            code: 's2'
        },
        {
            turkishText: 'ṣ',
            arabicText: 'ص',
            code: 's3'
        },
        {
            turkishText: 'ŝ',
            arabicText: 'ث',
            code: 's5'
        }
    ],
    'S': [
        {
            turkishText: 'S',
            arabicText: 'س',
            code: 'S'
        },
        {
            turkishText: 'Ş',
            arabicText: 'ش',
            code: 'S2'
        },
        {
            turkishText: 'Ṣ',
            arabicText: 'ص',
            code: 'S3'
        },
        {
            turkishText: 'Ŝ',
            arabicText: 'ث',
            code: 'S5'
        }
    ],
    't': [
        {
            turkishText: 't', // default
            arabicText: 'ت', //include d7
            code: 't'
        },
        {
            turkishText: 'ṭ',
            arabicText: 'ط',
            code: 't3'
        },
        {
            turkishText: 'd',
            arabicText: 'ط',
            code: 't6'
        },
        {
            turkishText: 't',
            arabicText: 'ة',
            code: 't7'
        }
    ],
    'T': [
        {
            turkishText: 'T',
            arabicText: 'ت',
            code: 'T'
        },
        {
            turkishText: 'Ṭ',
            arabicText: 'ط',
            code: 'T3'
        },
        {
            turkishText: 'D',
            arabicText: 'ط',
            code: 'T6'
        },
        {
            turkishText: 'T',
            arabicText: 'ة',
            code: 'T7'
        }
    ],
    'v': [
        {
            turkishText: 'v',
            arabicText: 'و',
            code: 'v'
        }
    ],
    'V': [
        {
            turkishText: 'V',
            arabicText: 'و',
            code: 'V'
        }
    ],
    'y': [
        {
            turkishText: 'y',
            arabicText: 'ي',
            code: 'y'
        }
    ],
    'Y': [
        {
            turkishText: 'Y',
            arabicText: 'ي',
            code: 'Y'
        }
    ],
    'z': [
        {
            turkishText: 'z', // default
            arabicText: 'ز',
            code: 'z'
        },
        {
            turkishText: 'ẓ',
            arabicText: 'ظ',
            code: 'z3'
        },
        {
            turkishText: 'ż',
            arabicText: 'ض',
            code: 'z4'
        },
        {
            turkishText: 'ẕ',
            arabicText: 'ذ',
            code: 'z5'
        }
    ],
    'Z': [
        {
            turkishText: 'Z',
            arabicText: 'ز',
            code: 'Z'
        },
        {
            turkishText: 'Ẓ',
            arabicText: 'ظ',
            code: 'Z3'
        },
        {
            turkishText: 'Ż',
            arabicText: 'ض',
            code: 'Z4'
        },
        {
            turkishText: 'Ẕ',
            arabicText: 'ذ',
            code: 'Z5'
        }
    ],
    'a': [
        {
            turkishText: 'a', // default
            arabicText: 'ﺃ',
            code: 'a0'
        },
        {
            turkishText: 'ā',
            arabicText: 'ﺃ',
            code: 'a1'
        }
    ],
    'A': [
        {
            turkishText: 'A',
            arabicText: 'ﺃ',
            code: 'A0'
        },
        {
            turkishText: 'Ā',
            arabicText: 'ﺃ',
            code: 'a1'
        }
    ],
    'e': [
        {
            turkishText: 'e', // default
            arabicText: 'ه',
            code: 'e0'
        },
        {
            turkishText: 'ė',
            arabicText: 'ﺽ',
            code: 'e1'
        },
        {
            turkishText: 'e',
            arabicText: 'ﺃ',
            code: 'e5'
        }
    ],
    'E': [
        {
            turkishText: 'E',
            arabicText: 'ه',
            code: 'E0'
        },
        {
            turkishText: 'ė', // ? No capital E with diacritic in chart
            arabicText: 'ﺽ',
            code: 'E1'
        },
        {
            turkishText: 'E',
            arabicText: 'ﺃ',
            code: 'E5'
        }
    ],
    'i': [
        {
            turkishText: 'ı',
            arabicText: 'ي',
            code: 'i0'
        },
        {
            turkishText: 'ī',
            arabicText: 'ي',
            code: 'i1'
        },
        {
            turkishText: 'i',
            arabicText: 'ى',
            code: 'i4'
        },
        {
            turkishText: 'i',
            arabicText: 'ي',
            code: 'i6'
        },
        {
            turkishText: 'ı',
            arabicText: 'آ',
            code: 'i7'
        },
        {
            turkishText: '', // ?
            arabicText: '', // ?
            code: 'i8'
        }
    ],
    'I': [
        {
            turkishText: 'I',
            arabicText: 'ي',
            code: 'I0'
        },
        {
            turkishText: 'Ī',
            arabicText: 'ي',
            code: 'I1'
        },
        {
            turkishText: 'I',
            arabicText: 'ى',
            code: 'I4'
        },
        {
            turkishText: 'I',
            arabicText: 'ي',
            code: 'I6'
        },
        {
            turkishText: 'I',
            arabicText: 'آ',
            code: 'I7'
        },
        {
            turkishText: '', // ?
            arabicText: '', // ?
            code: 'I8'
        }
    ],
    'o': [
        {
            turkishText: 'ö',
            arabicText: 'و',
            code: 'o0'
        },
        {
            turkishText: 'o',
            arabicText: 'ا',
            code: 'o2'
        },
        {
            turkishText: 'o',
            arabicText: 'و',
            code: 'o6'
        }
    ],
    'O': [
        {
            turkishText: 'Ö',
            arabicText: 'و',
            code: 'O0'
        },
        {
            turkishText: 'O',
            arabicText: 'ا',
            code: 'O2'
        },
        {
            turkishText: 'O',
            arabicText: 'و',
            code: 'O6'
        }
    ],
    'u': [
        {
            turkishText: 'ū',
            arabicText: 'و',
            code: 'u1'
        },
        {
            turkishText: 'ü',
            arabicText: 'و',
            code: 'u2'
        },
        {
            turkishText: 'u',
            arabicText: 'ا',
            code: 'u4'
        },
        {
            turkishText: 'u',
            arabicText: 'و',
            code: 'u6'
        },
        {
            turkishText: 'u',
            arabicText: 'ى',
            code: 'u8'
        },
        {
            turkishText: 'ü',
            arabicText: 'ى',
            code: 'u9'
        }
    ],
    'U': [
        {
            turkishText: 'Ū',
            arabicText: 'و',
            code: 'U1'
        },
        {
            turkishText: 'Ü',
            arabicText: 'و',
            code: 'U2'
        },
        {
            turkishText: 'U',
            arabicText: 'ا',
            code: 'U4'
        },
        {
            turkishText: 'U',
            arabicText: 'و',
            code: 'U6'
        },
        {
            turkishText: 'U',
            arabicText: 'ى',
            code: 'U8'
        },
        {
            turkishText: 'Ü',
            arabicText: 'ى',
            code: 'U9'
        }
    ],
    '\'': [
        {
            turkishText: 'ʿ',
            arabicText: 'ﻉ',
            code: '\'2'
        },
        {
            turkishText: '\'',
            arabicText: 'ﺃ',
            code: '\'3'
        }
    ]
}

export const turkishKeyboardDisambiguations = {
    'b': [
        {
            turkishText: 'b',
            arabicText: 'ب',
            code: 'b'
        },
        {
            turkishText: 'p',
            arabicText: 'ب',
            code: 'b7'
        }
    ],
    'B': [
        {
            turkishText: 'B',
            arabicText: 'ب',
            code: 'B'
        },
        {
            turkishText: 'P',
            arabicText: 'ب',
            code: 'B7'
        }
    ],
    'c': [
        {
            turkishText: 'c',
            arabicText: 'ﺝ',
            code: 'c'
        }
    ],
    'C': [
        {
            turkishText: 'C',
            arabicText: 'ﺝ',
            code: 'C'
        }
    ],
    'ç': [
        {
            turkishText: 'ç',
            arabicText: 'چ',
            code: 'c2'
        },
        {
            turkishText: 'ç',
            arabicText: 'ج',
            code: 'c7'
        }
    ],
    'Ç': [
        {
            turkishText: 'Ç',
            arabicText: 'چ',
            code: 'C2'
        },
        {
            turkishText: 'Ç',
            arabicText: 'ج',
            code: 'C7'
        }
    ],
    'd': [
        {
            turkishText: 'd',
            arabicText: 'د',
            code: 'd'
        },
        {
            turkishText: 'ḍ',
            arabicText: 'ض',
            code: 'd3'
        },
        {
            turkishText: 't',
            arabicText: 'د',
            code: 'd7'
        }
    ],
    'D': [
        {
            turkishText: 'D',
            arabicText: 'د',
            code: 'D'
        },
        {
            turkishText: 'Ḍ',
            arabicText: 'ض',
            code: 'D3'
        },
        {
            turkishText: 'T',
            arabicText: 'د',
            code: 'D7'
        }
    ],
    'f': [
        {
            turkishText: 'f',
            arabicText: 'ف',
            code: 'f'
        }
    ],
    'F': [
        {
            turkishText: 'F',
            arabicText: 'ف',
            code: 'F'
        }
    ],
    'g': [
        {
            turkishText: 'ġ', // default
            arabicText: 'غ',
            code: 'g4'
        },
        {
            turkishText: 'k',
            arabicText: 'ك',
            code: 'g7'
        }
    ],
    'G': [
        {
            turkishText: 'Ġ',
            arabicText: 'غ',
            code: 'G4'
        },
        {
            turkishText: 'K',
            arabicText: 'ك',
            code: 'G7'
        }
    ],
    'ğ': [
        {
            turkishText: 'ğ',
            arabicText: 'غ',
            code: 'g5'
        },
        {
            turkishText: 'ğ', // default
            arabicText: 'ك',
            code: 'g6'
        }
    ],
    'Ğ': [
        {
            turkishText: 'Ğ',
            arabicText: 'غ',
            code: 'G5'
        },
        {
            turkishText: 'Ğ',
            arabicText: 'ك',
            code: 'G6'
        }
    ],
    'h': [
        {
            turkishText: 'h',
            arabicText: 'ه',
            code: 'h'
        },
        {
            turkishText: 'ḥ',
            arabicText: 'ح',
            code: 'h3'
        },
        {
            turkishText: 'ẖ',
            arabicText: 'خ',
            code: 'h5'
        }
    ],
    'H': [
        {
            turkishText: 'H',
            arabicText: 'ه',
            code: 'H'
        },
        {
            turkishText: 'Ḥ',
            arabicText: 'ح',
            code: 'H3'
        },
        {
            turkishText: 'Ḫ',
            arabicText: 'خ',
            code: 'H5'
        }
    ],
    'j': [
        {
            turkishText: 'j',
            arabicText: 'ژ',
            code: 'j'
        }
    ],
    'J': [
        {
            turkishText: 'J',
            arabicText: 'ژ',
            code: 'J'
        }
    ],
    'k': [
        {
            turkishText: 'k',
            arabicText: 'ك',
            code: 'k'
        },
        {
            turkishText: 'ḳ',
            arabicText: 'ق',
            code: 'k3'
        }
    ],
    'K': [
        {
            turkishText: 'K',
            arabicText: 'ك',
            code: 'K'
        },
        {
            turkishText: 'Ḳ',
            arabicText: 'ق',
            code: 'K3'
        }
    ],
    'l': [
        {
            turkishText: 'l',
            arabicText: 'ل',
            code: 'l'
        }
    ],
    'L': [
        {
            turkishText: 'L',
            arabicText: 'ل',
            code: 'L'
        }
    ],
    'm': [
        {
            turkishText: 'm',
            arabicText: 'م',
            code: 'm'
        }
    ],
    'M': [
        {
            turkishText: 'M',
            arabicText: 'م',
            code: 'M'
        }
    ],
    'n': [
        {
            turkishText: 'n',
            arabicText: 'ن',
            code: 'n'
        },
        {
            turkishText: 'ñ',
            arabicText: 'ك',
            code: 'n6'
        }
    ],
    'N': [
        {
            turkishText: 'N',
            arabicText: 'ن',
            code: 'N'
        },
        {
            turkishText: 'Ñ',
            arabicText: 'ك',
            code: 'N6'
        }
    ],
    'p': [
        {
            turkishText: 'p',
            arabicText: 'ﭖ',
            code: 'p'
        }
    ],
    'P': [
        {
            turkishText: 'P',
            arabicText: 'ﭖ',
            code: 'P'
        }
    ],
    'r': [
        {
            turkishText: 'r',
            arabicText: 'ر',
            code: 'r'
        }
    ],
    'R': [
        {
            turkishText: 'R',
            arabicText: 'ر',
            code: 'R'
        }
    ],
    's': [
        {
            turkishText: 's', // default
            arabicText: 'س',
            code: 's'
        },
        {
            turkishText: 'ṣ',
            arabicText: 'ص',
            code: 's3'
        },
        {
            turkishText: 'ŝ',
            arabicText: 'ث',
            code: 's5'
        }
    ],
    'S': [
        {
            turkishText: 'S',
            arabicText: 'س',
            code: 'S'
        },
        {
            turkishText: 'Ṣ',
            arabicText: 'ص',
            code: 'S3'
        },
        {
            turkishText: 'Ŝ',
            arabicText: 'ث',
            code: 'S5'
        }
    ],
    'ş': [
        {
            turkishText: 'ş',
            arabicText: 'ش',
            code: 's2'
        }
    ],
    'Ş': [
        {
            turkishText: 'Ş',
            arabicText: 'ش',
            code: 'S2'
        }
    ],
    't': [
        {
            turkishText: 't',
            arabicText: 'ت',
            code: 't'
        },
        {
            turkishText: 'ṭ',
            arabicText: 'ط',
            code: 't3'
        },
        {
            turkishText: 'd',
            arabicText: 'ط',
            code: 't6'
        },
        {
            turkishText: 't',
            arabicText: 'ة',
            code: 't7'
        }
    ],
    'T': [
        {
            turkishText: 'T',
            arabicText: 'ت',
            code: 'T'
        },
        {
            turkishText: 'Ṭ',
            arabicText: 'ط',
            code: 'T3'
        },
        {
            turkishText: 'D',
            arabicText: 'ط',
            code: 'T6'
        },
        {
            turkishText: 'T',
            arabicText: 'ة',
            code: 'T7'
        }
    ],
    'v': [
        {
            turkishText: 'v',
            arabicText: 'و',
            code: 'v'
        }
    ],
    'V': [
        {
            turkishText: 'V',
            arabicText: 'و',
            code: 'V'
        }
    ],
    'y': [
        {
            turkishText: 'y',
            arabicText: 'ي',
            code: 'y'
        }
    ],
    'Y': [
        {
            turkishText: 'Y',
            arabicText: 'ي',
            code: 'Y'
        }
    ],
    'z': [
        {
            turkishText: 'z',
            arabicText: 'ز',
            code: 'z'
        },
        {
            turkishText: 'ẓ',
            arabicText: 'ظ',
            code: 'z3'
        },
        {
            turkishText: 'ż',
            arabicText: 'ض',
            code: 'z4'
        },
        {
            turkishText: 'ẕ',
            arabicText: 'ذ',
            code: 'z5'
        }
    ],
    'Z': [
        {
            turkishText: 'Z',
            arabicText: 'ز',
            code: 'Z'
        },
        {
            turkishText: 'Ẓ',
            arabicText: 'ظ',
            code: 'Z3'
        },
        {
            turkishText: 'Ż',
            arabicText: 'ض',
            code: 'Z4'
        },
        {
            turkishText: 'Ẕ',
            arabicText: 'ذ',
            code: 'Z5'
        }
    ],
    'a': [
        {
            turkishText: 'a',
            arabicText: 'ﺃ',
            code: 'a0'
        },
        {
            turkishText: 'ā',
            arabicText: 'ﺃ',
            code: 'a1'
        }
    ],
    'A': [
        {
            turkishText: 'Ā', // default
            arabicText: 'ﺃ',
            code: 'A0'
        },
        {
            turkishText: 'ā',
            arabicText: 'ﺃ',
            code: 'A1'
        }
    ],
    'e': [
        {
            turkishText: 'e', // default
            arabicText: 'ه',
            code: 'e0'
        },
        {
            turkishText: 'ė',
            arabicText: 'ﺽ',
            code: 'e1'
        },
        {
            turkishText: 'e',
            arabicText: 'ﺃ',
            code: 'e5'
        }
    ],
    'E': [
        {
            turkishText: 'E',
            arabicText: 'ه',
            code: 'E0'
        },
        {
            turkishText: 'ė', // ? No E with a diacritic
            arabicText: 'ﺽ',
            code: 'E1'
        },
        {
            turkishText: 'E',
            arabicText: 'ﺃ',
            code: 'E5'
        }
    ],
    'i': [
        {
            turkishText: 'ı', // own character on Turkish keyboard, default
            arabicText: 'ي',
            code: 'i0'
        },
        // i2 without a dot, own character in Turkish
        {
            turkishText: 'i',
            arabicText: 'ى',
            code: 'i4'
        },
        {
            turkishText: 'i', // default
            arabicText: 'ي',
            code: 'i6'
        },
        {
            turkishText: 'ı', // own character
            arabicText: 'آ',
            code: 'i7'
        },
        {
            turkishText: '', // ?
            arabicText: '', // ?
            code: 'i8'
        }
    ],
    'I': [
        {
            turkishText: 'I',
            arabicText: 'ي',
            code: 'I0'
        },
        {
            turkishText: 'I',
            arabicText: 'ى',
            code: 'I4'
        },
        {
            turkishText: 'I',
            arabicText: 'ي',
            code: 'I6'
        },
        {
            turkishText: 'I',
            arabicText: 'آ',
            code: 'I7'
        },
        {
            turkishText: '', // ?
            arabicText: '', // ?
            code: 'I8'
        }
    ],
    'ī': [ // appear under i without a dot and i with a dot
        {
            turkishText: 'ī',
            arabicText: 'ي',
            code: 'i1'
        }
    ],
    'Ī': [
        {
            turkishText: 'Ī',
            arabicText: 'ي',
            code: 'I1'
        }
    ],
    'o': [
        {
            turkishText: 'o',
            arabicText: 'ا',
            code: 'o2'
        },
        {
            turkishText: 'o', // default
            arabicText: 'و',
            code: 'o6'
        }
    ],
    'O': [
        {
            turkishText: 'O',
            arabicText: 'ا',
            code: 'O2'
        },
        {
            turkishText: 'O',
            arabicText: 'و',
            code: 'O6'
        }
    ],
    'ö': [
        {
            turkishText: 'ö', // default
            arabicText: 'و',
            code: 'o0'
        }
    ],
    'Ö': [
        {
            turkishText: 'Ö',
            arabicText: 'و',
            code: 'O0'
        }
    ],
    'u': [
        {
            turkishText: 'ū',
            arabicText: 'و',
            code: 'u1'
        },
        {
            turkishText: 'u',
            arabicText: 'ا',
            code: 'u4'
        },
        {
            turkishText: 'u', // default
            arabicText: 'و',
            code: 'u6'
        },
        {
            turkishText: 'u',
            arabicText: 'ى',
            code: 'u8'
        }
    ],
    'U': [
        {
            turkishText: 'Ū',
            arabicText: 'و',
            code: 'U1'
        },
        {
            turkishText: 'U',
            arabicText: 'ا',
            code: 'U4'
        },
        {
            turkishText: 'U',
            arabicText: 'و',
            code: 'U6'
        },
        {
            turkishText: 'U',
            arabicText: 'ى',
            code: 'U8'
        }
    ],
    'ü': [
        {
            turkishText: 'ü', // default 7
            arabicText: 'و',
            code: 'u2'
        },
        {
            turkishText: 'ü',
            arabicText: 'ى',
            code: 'u9'
        }
    ],
    'Ü': [
        {
            turkishText: 'Ü',
            arabicText: 'و',
            code: 'U2'
        },
        {
            turkishText: 'Ü',
            arabicText: 'ى',
            code: 'U9'
        }
    ],
    '\'': [
        {
            turkishText: 'ʿ',
            arabicText: 'ﻉ',
            code: '\'2'
        },
        {
            turkishText: '\'',
            arabicText: 'ﺃ',
            code: '\'3'
        }
    ]
}